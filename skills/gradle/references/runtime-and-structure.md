...
- When user-home evidence differs, check wrapper `-g`/`--gradle-user-home`, `-Dgradle.user.home`, project `systemProp.gradle.user.home`, and `GRADLE_USER_HOME`; the wrapper resolves user home before reading user `gradle.properties` and ignores `systemProp.gradle.user.home` from that user home with a warning.
