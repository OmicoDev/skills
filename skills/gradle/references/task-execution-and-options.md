...
- Keep `@Option` names unique by design. Gradle 9.7 collapses repeated declarations of the same method signature but fails option discovery when one name maps to different method signatures, so do not rely on discovery failure to detect every duplicate; verify inherited options with `help --task`.
...
- On Gradle 9.7, documented `@Option` types are `boolean`/`Boolean`/`Property<Boolean>`; `Double`, `Integer`, `Long`, `String`, or enum values and their matching `Property` forms; `List<T>`, `ListProperty<T>`, or `SetProperty<T>` for those non-boolean scalar and enum types; and `DirectoryProperty` or `RegularFileProperty`. Recheck the manual for another Gradle line; use extension properties, project properties, or build services for complex or cross-task build knobs.
...
- Interpret task outcome labels before changing wiring: no label/`EXECUTED` means actions ran or a no-action lifecycle task had executed dependencies; `UP-TO-DATE` means outputs did not change, or a no-action task had no dependencies or all dependencies ended `UP-TO-DATE`, `SKIPPED`, or `FROM-CACHE`; `FROM-CACHE` restored outputs; `SKIPPED` skipped actions; and `NO-SOURCE` found no source inputs.
