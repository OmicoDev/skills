...
- Since Gradle 4.5, ordinary composite included builds use the top-level build's build-cache configuration and ignore their own cache blocks; since Gradle 6.0, the same inheritance applies to `buildSrc` builds. Exclude plugin builds included through `pluginManagement`: they are loaded before the top-level cache configuration is available and can use their own cache configuration, so diagnose their backend separately.
