...
- Gradle 9.7+ logs TestNG, JUnit 4, and JUnit Platform framework-startup failures even when test logging granularity would hide them; explicitly omitting `FAILED` from `testLogging.events` still suppresses them. Tooling API and Build Scan consumers can distinguish them with the incubating `TestFailureDetails.isFrameworkFailure()` predicate.
- Keep the Gradle wrapper aligned with TestNG when configuring `threadPoolFactoryClass`: the option exists on Gradle 8.7+, but Gradle 8.7 through 9.6.1 only invokes TestNG's legacy `setExecutorFactoryClass(String)`, so setting it with TestNG 7.10+ fails with Gradle's unsupported-version error. Gradle 9.7+ detects the TestNG API; factories for TestNG 7.10+ must implement `org.testng.IExecutorServiceFactory`, while TestNG 7.0 through 7.9 requires `org.testng.thread.IExecutorFactory`. Give the factory class a public no-arg constructor. The build-script property is unchanged; when adopting TestNG 7.10+, upgrade the wrapper to Gradle 9.7+ or leave the custom factory unset.
...
- `executionData(task)` adds the task's JaCoCo data file and ordering, not a task dependency. Both report and verification tasks skip when none of their execution-data files exists; wire the producing test task when current coverage is required, and treat a skipped coverage gate as missing evidence rather than a pass.
- Keep JaCoCo scope owners separate: execution data chooses covered runs; `classDirectories` plus `additionalClassDirs` choose measured classes; `sourceDirectories` plus `additionalSourceDirs` only locate corresponding sources. Apply bytecode filters to the complete class-directory union on every report and verification task that must share policy.
- Coverage rules fail on violation by default. `failOnViolation = false` logs violations when the task executes while allowing success, so reserve it for rollout evidence rather than a release gate.
- On Gradle 7.1+, `JacocoCoverageVerification` is build-cacheable through an internal success marker. Its omission from `check` is an opt-in policy boundary, not evidence that the task cannot reuse a successful result; remember that a lenient `failOnViolation = false` result can also be reused without re-emitting its warning.
...
- On Gradle 9.4+, non-class-based engines such as Cucumber can use the incubating `testDefinitionDirs` input only through JUnit Platform. Add the runtime `TestEngine` and prefer a separate `Test` task or suite from class-based tests; non-empty definition directories can run without compiled test classes, while an otherwise source-less task is skipped.
- Non-class test filtering is file-level and derives dotted names from paths, so dotted directories or filenames can make individual definitions ambiguous. `maxParallelForks` distributes definition directories, not files inside one directory; add independently safe directories when process-level parallelism matters.
- On Gradle 9.4+, JUnit Platform key-value report entries become structured test metadata. File attachments also require framework-owned `FileEntry` support: Jupiter `TestReporter.publishFile(...)` and `publishDirectory(...)` require Jupiter 5.12+, while another engine must emit `FileEntry` events before Gradle can capture attachments.
- Gradle 9.4+ HTML reports expose captured data and attachments, while JUnit XML writes key-values as `<property>` elements and attachment markers into `system-out`; archive the referenced files with the report instead of assuming XML embeds their bytes.
- Use the incubating task-scoped `addTestMetadataListener(...)` when build logic must react to Gradle 9.4+ structured metadata, and branch on `TestKeyValueDataEvent` versus `TestFileAttachmentDataEvent`. Prefer this over parsing standard output or registering a build-wide listener.
