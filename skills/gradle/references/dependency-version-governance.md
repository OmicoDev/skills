...
- Catalog TOML cannot encode classifiers, artifact types, excludes, or capabilities. On Gradle 6.8+, derive a classifier request at the use site with `variantOf(libs.alias) { classifier(...) }`; this returns a new provider and leaves the catalog alias unchanged. Configure a specific artifact type in the dependency's `artifact {}` block.
- Reserve classifier or explicit artifact-type requests for artifacts genuinely published that way. Use `testFixtures(libs.alias)`, `platform(libs.alias)`, attributes, or metadata rules when the request represents test fixtures, a platform, or another semantic variant, and put excludes or capabilities at their dependency/metadata owner.
...
- When an imported catalog artifact is locked, lockfile entries can reference `incomingCatalogFor<Name>0`; that configuration records the external catalog artifact resolution, not dependency versions named inside the catalog.
