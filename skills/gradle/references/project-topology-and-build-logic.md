...
- Project names must not contain `/`, `\`, `:`, `<`, `>`, `"`, `?`, `*`, or `|`, and must not start or end with `.`; set `rootProject.name`, `include(...)`, or project descriptor names instead of inheriting invalid directory names.
