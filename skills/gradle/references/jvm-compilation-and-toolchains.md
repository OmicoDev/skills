...
- Toolchain coverage differs by JVM plugin: Java covers compile, test, and Javadoc; Groovy covers compilation and, on Gradle 9.7+, Groovydoc; Scala covers compilation and Scaladoc. Groovydoc on earlier Gradle versions still runs on the Gradle runtime JVM.
...
- On Gradle 9.7+, `Groovydoc` defaults to the project's Java toolchain and runs in a process-isolated worker; its new task-level `javaLauncher` and `maxMemory` properties are incubating, with `maxMemory` controlling the worker heap. On earlier Gradle versions, keep the Gradle runtime JVM compatible with the Groovy implementation because Groovydoc is not toolchain-aware.
