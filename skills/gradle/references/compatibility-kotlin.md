...
- Plugins built and published with the `kotlin-dsl` plugin on Gradle 9 have a supported consumer floor of Gradle 8.11. Lowering `languageVersion` or `apiVersion` in that Gradle 9 build is not a supported path to older consumers; build the plugin with Gradle 8.x and cross-version-test the minimum consumer instead.
