...
- On Gradle 9.7+, configuration actions for `WorkParameters.None`, `TransformParameters.None`, `BuildServiceParameters.None`, `ValueSourceParameters.None`, `FlowParameters.None`, and `JvmTestToolchainParameters.None` always run and receive a singleton. For work actions, transforms, build services, value sources, and flow actions, `getParameters()` returns that singleton instead of `null`; remove null branches and do not rely on configuration being skipped.
