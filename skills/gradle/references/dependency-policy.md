...
- Gradle 9.7 promotes `DependencyHandler.project()`/`project(String)` and both `DependencyFactory.createProjectDependency(...)` overloads to stable APIs. They were introduced in Gradle 9.5, so plugins with an older consumer floor still need a version-compatible declaration path.
