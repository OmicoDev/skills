...
- Put required user action in labels, details, documented links, locations, and solutions. Use additional data for tool clients, not as the only human-readable fix.
...
- For Tooling API failure checks, register a root-operation progress listener or request detailed failures, then verify `Failure.getProblems()` rather than console text alone.
- Test fatal and recoverable paths separately because `throwing(...)` changes task/build outcome.
- For duplicate problems, assert one representative problem plus summarization behavior rather than every repeated instance appearing as a full console event.
...
Primary upstream pages: Reporting Plugin Problems with the Problems API, Problems HTML Report, Tooling API problem events.
