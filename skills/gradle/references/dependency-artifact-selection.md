...
- Treat artifact view attributes as live until resolution. Later changes to the backing configuration attributes or provider-backed view attributes can affect the files selected by the view.
- When artifact order is part of the task contract, preserve the order returned by the resolvable configuration, `incoming.files`, or artifact view; use `resolutionStrategy.sortArtifacts(...)` only when consumer-first or dependency-first artifact order is intentional, and do not expect strict ordering guarantees through dependency cycles.
- Gradle 9.7's `ENHANCED_GRAPH_ORDERING` feature preview adopts Gradle 10's planned ordering: `DEFAULT` is breadth-first, `CONSUMER_FIRST` is topological, `DEPENDENCY_FIRST` reverses it, and every mode ignores dependency-constraint edges. Test classpath-sensitive builds before enabling it.
- `ArtifactCollection` can expose duplicate local artifact metadata that the corresponding file collection de-duplicates; use files for file-only task inputs and artifact metadata only when duplicates or component identity matter.
...
- Wire provider-backed artifact results and `artifactFiles` into task properties so non-selected tasks can run without resolving artifacts; a successful `help` run proves deferred wiring, not dependency health.
- For configuration-cache-compatible tasks, split `ResolvedArtifactResult` data into stable file and metadata task properties instead of storing result objects as task inputs. On Gradle 9.7+, read artifact attributes and capabilities directly through `getAttributes()` and `getCapabilities()`; `getVariant()` remains available but is planned for deprecation.
...
- On Gradle 7.5+, use `withVariantReselection()` only when the consumer intentionally wants a parallel variant of each selected component, such as sources or javadoc. It remains an incubating API; provide the complete target variant attributes because the configuration's graph attributes are ignored during reselection.
- Variant reselection does not rerun graph resolution: it neither traverses dependencies declared only by the reselected variant nor follows Gradle Module Metadata `available-at` pointers to another component. When those components and their artifacts must enter the result, declare a separate resolvable configuration whose request attributes select the target variants during graph resolution.
...
- Sources or javadoc view is empty: check for dependencies declared with explicit artifact/classifier requests, missing documentation variants on selected capabilities, or artifact view attributes that request a non-existent artifact variant.
- Artifact view attributes do not select another project configuration: check whether the project dependency explicitly names a configuration or whether variant reselection is missing; artifact view attributes alone do not override the graph-selected configuration or variant.
