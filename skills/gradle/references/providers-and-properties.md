...
- Prefer dedicated collection properties over `Property<Collection<T>>`, `Property<Iterable<T>>`, arrays, or plain mutable collection fields. `ListProperty<T>` exists since Gradle 4.3 but gains additive `HasMultipleValues` methods in 4.5; `SetProperty<T>` exists since 4.5 and `MapProperty<K, V>` since 5.1. On Gradle 5.1+, all three accept independent lazy contributions without configuration-time read-copy-write and retain producer task dependencies from provider-backed elements or entries; use `ConfigurableFileCollection` or `ConfigurableFileTree` when the values are files.
...
- An added element, collection, entry, or map provider is required: if it is absent, the whole collection property is absent. On Gradle 5.6+, when absence should mean “contribute nothing,” map the optional value to an iterable or map and use `orElse` with an empty value before `addAll` or `putAll`. In Gradle 9.6+ Groovy DSL, assigning one matching element or a compatible object array to a list/set property replaces it with the corresponding one- or many-element collection; keep using additive methods when contributions must compose instead of inferring `+=` support.
...
- Prefer a directly annotated abstract output property such as `@OutputFile RegularFileProperty`. A derived getter returning `Provider` can lose producer provenance even when annotated; when compatibility with such a task type is unavoidable, keep the outer task relationship with `taskProvider.map { it.outputFile.get() }` rather than `flatMap`, and do not generalize that localized bridge into configuration-time eager reads.
...
- Do not implement `Provider`, `Property`, or their subtypes yourself; obtain them from Gradle factories, layouts, tasks, or other model objects and expose them directly through getters instead of hiding lazy wiring behind wrapper getters/setters that call `.get()` or `.set(...)`.
...
- Model custom `ValueSourceParameters` as interfaces with property-like getters; put cache-significant inputs in parameters, let Gradle supply `getParameters()` instead of implementing it, and keep `obtain()` fast with effectively immutable results because queried value sources run on every build to decide configuration-cache reuse.
