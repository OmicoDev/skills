...
- The ancestor leg is conditional: Vintage builds continue through ancestor extra properties and extensions only while implicit parent lookup is enabled, whereas Isolated Projects omit it and Gradle 9.6's `NO_IMPLICIT_LOOKUP_IN_PARENT_PROJECTS` feature preview disables it. Gradle 8.x can also expose legacy convention-plugin properties after extensions in the current and inherited scopes. Use the version- and mode-specific order diagnostically, and remove reliance on ancestor lookup as described in [scripts-and-conventions.md](scripts-and-conventions.md).
