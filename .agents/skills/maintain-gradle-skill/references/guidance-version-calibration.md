...
- Treat a future target named by a final release's deprecation contract as a schedule, not observed target-version behavior. Keep the released warning and planned target together, label the target behavior as planned, and recheck it when that target has a final release.
- When auditing deprecations, inspect direct source and representative tests for schedule markers such as `willBecomeAnErrorInGradle*`, `willBeRemovedInGradle*`, and emitted “will fail” messages. Record the current released warning, the planned target behavior, and the target-final recheck together; the marker proves the schedule carried by its released host, not the target release's implementation.
